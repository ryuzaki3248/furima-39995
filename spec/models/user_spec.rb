...
#       @user.valid?
#       expect(@user.errors.full_messages).to include("Full_width_last_name can't be blank")
#     end
#     it "full_width_first_nameが空では登録できない" do
#       @user.full_width_first_name = ''
#       @user.valid?
#       expect(@user.errors.full_messages).to include("Full_width_first_name can't be blank")
#     end
#     it "full_width_last_name_kanaが空では登録できない" do
#       @user.full_width_last_name_kana = ''
#       @user.valid?
#       expect(@user.errors.full_messages).to include("Full_width_last_name_kana can't be blank")
#     end
#     it "full_width_first_name_kanaが空では登録できない" do
#       @user.full_width_first_name_kana = ''
#       @user.valid?
#       expect(@user.errors.full_messages).to include("Full_width_first_name_kana can't be blank")
#     end
...
RSpec.describe User, type: :model do
  ...
  describe 'ユーザー新規登録' do
    context '新規登録できる場合' do
      ...
      it '新規登録できる場合' do
        @user.full_width_last_name = '山田'
        @user.full_width_first_name = '太郎'
        @user.full_width_last_name_kana = 'ヤマダ'
        @user.full_width_first_name_kana = 'タロウ'
        @user.birth_date = Date.new(1992, 3, 25)
        expect(@user).to be_valid
      end
    ...
    context '新規登録できない場合' do
      ...
      it '重複したemailが存在する場合は登録できない' do
        ...
        begin
          ...
          expect(e.record.errors.full_messages).to include(
            "Full width last name must be full-width characters in hiragana, katakana, or kanji",
            "Full width first name must be full-width characters in hiragana, katakana, or kanji",
            "Full width last name kana must be full-width katakana characters",
            "Full width first name kana must be full-width katakana characters"
          )
        end
      end
